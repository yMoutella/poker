        ...
        <Card className="w-64 border-dashed border-2 bg-chart-6 rounded-xl flex flex-col" >
            ...
            <CardContent className="py-6 text-sidebar-accent-foreground font-mono text-sm justify-center overflow-scroll max-h-96 shadow-inner-2xs rounded-2xl">
                {stories.map((story: Story, i) => (
                    <Card key={i} className="bg-card p-5 mb-2 border-border rounded-md cursor-pointer focus:border-accent-foreground hover:bg-accent hover:border-accent-foreground hover:text-accent-foreground" onClick={() => setStory(story)}>
            ...
            </CardContent>
            {
                stories.length > 0 ? (
                    <div className="w-full flex justify-center p-2 mt-4">
                        <Label className="cursor-pointer text-amber-400 hover:text-amber-600 text-2xl">History<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-8">
